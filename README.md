...
A unified [Claude Code](https://claude.com/claude-code) skill for everything Kaggle: account setup, competition landscape reports, dataset/model downloads, notebook execution, competition submissions, badge collection, and general Kaggle questions.
...
- Optional: Playwright MCP tools in Claude Code (for competition report scraping)

## Usage

Once installed, Claude Code automatically detects the skill. Invoke it by asking anything Kaggle-related:
...
├── .claude-plugin/marketplace.json # Anthropic Marketplace config
└── tests/                          # Test suite
```
